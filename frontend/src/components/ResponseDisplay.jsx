          ...
          {response.invalid_entries?.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {response.invalid_entries.map((item, idx) => (
                <motion.span 
                  initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + (idx * 0.05) }}
                  key={idx} 
                  className="px-3 py-1 bg-rose-500/10 border border-rose-500/20 text-rose-300 rounded-lg text-sm font-mono shadow-sm"
                >
                  {item}
                </motion.span>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {response.duplicate_edges.map((item, idx) => (
                <motion.span 
                  initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + (idx * 0.05) }}
                  key={idx} 
                  className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-300 rounded-lg text-sm font-mono shadow-sm"
                >
                  {item}
                </motion.span>
              ))}
      ...
      <motion.div variants={itemVariants} className="glass-card p-6 overflow-hidden">
        ...
        {response.hierarchies?.has_cycle && (
          <motion.div 
             ...
             className="mb-6 p-4 bg-rose-500/10 border border-rose-500/30 rounded-xl flex items-center gap-3 shadow-sm"
          >
